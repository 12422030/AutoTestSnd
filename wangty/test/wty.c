...
int wty(TxipCMStc *xip)
{
    ...
    strcpy(filename,"wty.txt");
    sprintf(filePath, "%s/src/test/wangty/test/%s",getenv("HOME"),filename);
	...
	if(curNode!=NULL)
    {
		...
		while(node2 !=  NULL)
		{
            ...
            APPLOG("D","11111111111111111111");
			ret = Epcc_clearfile_Ins( epcc_clearfile, Mxip_rc);
           APPLOG("D","222222222222222222222222222"); 
			if(ret !=0)
			{
            ...
            APPLOG("D","33333333333333333333333333");
			APPLOG("D", "汇总行插入完毕");
			//**9.BatchList
			curNode = curNode->next;
			APPLOG("D","curNode-[%s]", curNode->name);
