...
int	wys_rcv(int argc, char **argv)
{
	...
	while (1) 
	{
		...
		else if ( child_pid == 0 )/*子进程*/
		{
            ...
            memcpy( servicename, msg+8, 4);
            strcpy( servicename, "test");
           
            printf("11111111111\n");
           /* char *p=NULL; 
            p = strstr(msg, "SendNode");
            memcpy( servicename, p+1, 6);
            strcpy(servicename,"PA01");*/

            /* 打开报文文件 */
            sprintf( filename, "%s/src/test/vtmbank/wz/msg/%s",getenv("HOME"),servicename);
            printf("[%s]\n",filename);
            printf("11111111111\n");
            ...
            /**sprintf( buff, "%08ld%s", strlen(tmp), tmp);**/
            sprintf( buff, "%08ld",strlen(tmp)-1);
            printf("服务端发送的报文是:[%s]\n", buff);
            if ( send( newsock, buff, 8, 0)<0)
			{
                printf( "send信息出错!![%s]", newsock);
                exit(1);
            }
          /* if ( send( newsock, "00|45|00",8,0)<0)
            {   
                printf( "send信息出错!![%s]", newsock);
                exit(1);
            }*/
            if(send(newsock,tmp,strlen(tmp)-1,0)<0)
            {   
                printf( "send信息出错!![%s]", newsock);         
                exit(1);
            }
