...
int	wys_rcv(int argc, char **argv)
{
	...
	while (1) 
	{
		...
		else if ( child_pid == 0 )/*子进程*/
		{
            ...
            strcpy(servicename,"HF0017");
           
            printf("D","11111");
            /* 打开报文文件 */
            sprintf( filename, "%s/src/test/hf/msg/%s",getenv("HOME"),servicename);
            printf("[%s]\n",filename);
            fp = fopen( filename, "r");
            if (fp==NULL)
            {
            ...
            printf("返回报文是[%s]\n",tmp);
            long returnlen=0;
            returnlen=htonl(strlen(tmp));
         /*   
            if(send(newsock,"00000000000003130780",20,0)<0)
            {
                return -1;
            }*/

            printf("returnlen[%ld]\n",returnlen);
            sprintf( buff, "0000000000000313%04ld%s", returnlen, tmp);
             returnlen=0;
            returnlen=htonl(strlen(buff));
            if(send(newsock,buff,returnlen,0)<0)
            {   
                printf( "send信息出错!![%s][%ld]", newsock,returnlen);         
                exit(1);
            }
            /*if(send(newsock,buffer,strlen(buffer)-1,0)<0)*/
           /* if(send(newsock,buffer,returnlen,0)<0)
            {   
                printf( "send信息出错!![%s][%ld]", newsock,returnlen);         
                exit(1);
            }*/
